...
public class MethodBody extends CompilerModule {
  ...
  public MethodBody(FunctionLocals functionLocals, SimpleNode methodNode) {
    ...
    // construct DAGNodes - number nodes = N (Number of method body children) - M (number of var
    // declarations) to construct each DAG: new NodeFactory(locals).build(node);
    ...
    for (int i = 0; i < numberDeclExp; i++) {
      if (methodBodyNode.jjtGetChild(i).is(JJTVARIABLEDECLARATION)) numberDecl++;
    }
